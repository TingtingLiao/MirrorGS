...
def create_videos(base_dir, input_dir, out_name, num_frames=480):
  ...
  depth_file = os.path.join(input_dir, 'depth', f'depth_{idx_to_str(0)}.tiff')
  ...
  for k in ['normal', 'color']:
    ...
    if k == 'color':
      ...
      file0 = os.path.join(input_dir, k, f'{k}_{idx_to_str(0)}.{file_ext}')
    ...
    with media.VideoWriter(
      ...
      for idx in tqdm(range(num_frames)):
        ...
        if k == 'color':
          ...
          img_file = os.path.join(input_dir, k, f'{k}_{idx_to_str(idx)}.{file_ext}')

        if not os.path.exists(img_file):
          ValueError(f'Image file {img_file} does not exist.')
        img = load_img(img_file)
        if k in ['depth', 'color', 'normal']:
          img = img / 255.
...
def save_mask_u8(mask, pth):
    """Save a mask image in [0, 1] to disk as a uint8 PNG."""
    mask_u8 = (np.clip(np.nan_to_num(mask), 0., 1.) * 255).astype(np.uint8)
    Image.fromarray(mask_u8, mode='L').save(pth, 'PNG')
